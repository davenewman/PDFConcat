...
def concatenate_and_rotate(filenames, states):
    ...
    for info in information:
        with open(info[0], 'rb') as PDFFileObj:
            pdf_read_obj = PyPDF2.PdfFileReader(PDFFileObj)
            for pagenum in range(pdf_read_obj.numPages):
    ...
    for filename in filenames:
        with open(filename, 'rb') as PDFFileObj:
            ...
            for pagenum in range(pdf_read_obj.numPages):
                page = pdf_read_obj.getPage(pagenum)
                pdf_write_obj.addPage(page)
            output_obj = open('output_cat.pdf', 'ab')
            pdf_write_obj.write(output_obj)
            output_obj.close()
...
def rotate_within_pdf(filename, pagenumbers, states):
    ...
    with open(filename, 'rb') as PDFFileObj:
        ...
        for pagenum in range(pdf_read_obj.numPages):
            ...
            pdf_write_obj.addPage(page)
        output_obj = open('output_single.pdf', 'wb')
        pdf_write_obj.write(output_obj)
        output_obj.close()
